...
const SUGGESTED = [
  "My workflow keeps failing at the webhook step",
  "How do I check my AI credit balance?",
  "I can't log into my dashboard",
  "Getting 401 error on API calls",
];
...
function formatHistory(messages) {
  return messages
    .filter((m) => !["system","escalation","queue"].includes(m.from))
          ...
          <div className="grid grid-cols-3 gap-2">
            {[["Category", summary.category, "text-white"],
              ["Priority", capitalize(summary.priority),
                summary.priority === "critical" ? "text-red-400" : summary.priority === "high" ? "text-orange-400" : summary.priority === "low" ? "text-emerald-400" : "text-slate-300"],
              ["Sentiment", sentCfg.label, sentCfg.color]].map(([l, v, c]) => (
...
// Subtle agent badge — clean, not noisy
function AgentBadge({ agentInfo }) {
  if (!agentInfo?.agent) return null;
  const colors = AGENT_COLORS[agentInfo.color] || AGENT_COLORS.slate;
  return (
    <div className={`inline-flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full border mb-1.5 ${colors.bg} ${colors.text}`}>
      <span>{agentInfo.emoji}</span>
      <span className="font-medium">{agentInfo.agent}</span>
    </div>
  );
}
...
function Avatar({ from }) {
  return from === "user" ? (
    ...
    <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center shrink-0 text-base">⚡</div>
  );
}

// Clean escalation notice — no ticket IDs or queue numbers in customer view
function EscalationNotice() {
  return (
    <div className="mx-1 bg-slate-800 border border-orange-800 rounded-xl px-4 py-3 flex items-center gap-3">
      <span className="text-orange-400 text-lg">👋</span>
      <div>
        <p className="text-white text-sm font-medium">Connecting you to a specialist</p>
        <p className="text-slate-400 text-xs mt-0.5">A human support agent has been notified and will follow up shortly.</p>
      </div>
    </div>
  );
}

// Critical notice — slightly more urgent but still clean
function CriticalNotice({ queuePos }) {
  return (
    <div className="mx-1 bg-red-950 border border-red-800 rounded-xl px-4 py-3">
      <div className="flex items-center gap-2 mb-1.5">
        <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
        <p className="text-white text-sm font-semibold">Priority escalation — agent notified</p>
      </div>
      <p className="text-slate-400 text-xs">You're #{queuePos} in the support queue. Estimated response within {queuePos * 3} minutes.</p>
    </div>
  );
}
    ...
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} max-w-[78%]`}>
        ...
        <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed
          ...
          {msg.text}
          <div className={`text-[10px] mt-1.5 ${isUser ? "text-indigo-300 text-right" : "text-slate-500"}`}>{msg.time}</div>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  async function sendMessage(text_override) {
    ...
    try {
      ...
      // Build message list — clean notices instead of technical banners
      let newMsgs = [...updated];
  ...
  function clearChat() {
    setMessages([{
      id: 0, from: "bot", agentInfo: null,
      text: "Hi! I'm SupportFlow AI — your FlowZint enterprise support assistant.\n\nI can help you with workflows, API integrations, billing, platform access, and more. What's going on?",
      time: now(),
    }]);
    setError(null);
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      ...
      <header className="border-b border-slate-800 bg-slate-950">
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          {/* Feature pills — header only, not in chat */}
          <div className="hidden lg:flex items-center gap-2">
            {["⚡ Workflow AI","💳 Billing AI","🔌 API AI","🔐 Access AI"].map(b => (
              <span key={b} className="text-[11px] px-2.5 py-1 rounded-full border border-slate-700 bg-slate-900 text-slate-400">{b}</span>
            ))}
          </div>
      ...
      {/* ══ CHAT TAB — clean customer-facing view ════════════════ */}
      {tab === "chat" && (
        <div className="flex flex-1 overflow-hidden" style={{ height: "calc(100vh - 112px)" }}>
          <div className="flex-1 flex flex-col min-w-0 bg-slate-950">

            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-5 max-w-3xl mx-auto w-full">
              {messages.map((msg) => {
                if (msg.from === "escalation") return <EscalationNotice key={`esc-${msg.time}-${Math.random()}`} />;
              ...
              {error && (
                <div className="flex items-start gap-2 bg-red-950 border border-red-800 rounded-xl px-4 py-3 text-sm text-red-300 max-w-3xl mx-auto w-full">
              ...
              {messages.length === 1 && !loading && (
                <div className="max-w-3xl mx-auto w-full">
                  <p className="text-slate-600 text-xs mb-3 text-center">Common support queries</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {SUGGESTED.map((q) => (
                      <button key={q} onClick={() => sendMessage(q)}
                        className="text-left text-sm text-slate-400 bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 hover:border-indigo-600 hover:text-white transition-colors">
                        {q}
                      </button>
                    ))}
                  </div>
            ...
            {/* Input */}
            <div className="border-t border-slate-800 bg-slate-950 px-4 md:px-8 py-4">
              <div className="max-w-3xl mx-auto">
                <div className="flex gap-2 items-end bg-slate-900 border border-slate-700 rounded-2xl px-4 py-3 focus-within:border-indigo-500 transition-colors">
                  <textarea ref={inputRef} value={input}
                    onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
                    placeholder="Describe your issue — workflow errors, API problems, billing questions..."
                    rows={1} disabled={loading}
                    className="flex-1 bg-transparent text-slate-200 placeholder-slate-600 text-sm resize-none focus:outline-none disabled:opacity-50 max-h-32"
                    style={{ fieldSizing: "content" }} />
                  <button onClick={() => sendMessage()} disabled={!input.trim() || loading}
                    className="shrink-0 w-9 h-9 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700
                      disabled:text-slate-500 text-white rounded-xl flex items-center justify-center transition-colors">
                    {loading
                ...
                <div className="flex items-center justify-between mt-2 px-1">
                  <span className="text-slate-700 text-[11px]">Enter to send · Shift+Enter for new line</span>
                  <button onClick={clearChat} className="text-slate-600 hover:text-slate-400 text-[11px] transition-colors">Clear chat</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          {summaries.length === 0 ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-white font-semibold text-sm">AI Session Summaries <span className="text-slate-500 font-normal text-xs ml-1">({summaries.length})</span></h2>
      ...
      {tab === "queue" && (
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="bg-red-900 text-red-300 border border-red-700 text-xs px-2.5 py-1 rounded-full font-medium">{escalationQueue.length} waiting</span>
          </div>
                ...
                <div key={item.ticket_id} className="bg-slate-900 border border-red-800 rounded-2xl p-5 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      {(() => { const cfg = SENTIMENT_CONFIG[item.sentiment] || SENTIMENT_CONFIG.neutral; return (
